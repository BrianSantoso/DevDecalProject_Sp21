...
contract MerchToken is ERC721, Ownable {
    ...
    function mintToken(
        ...
        bytes32 hash1 = sha256(abi.encode(block.timestamp + uint256(msg.sender)));
        bytes32 hash2 = keccak256(abi.encode(hash1));
        uint256 result = uint256(hash2);

        uint256 _merchType = result % 5;
        uint256 _tag = result % 10;
    ...
    function changePrice(uint256 _tokenId, uint256 _newPrice) public {
        require(_newPrice > 0); // New price should be non-zero
        require(tokenInfos[_tokenId].price != 0); // Check that token with that id exists (if not, price is 0)
